...
import com.poojabhagat.Lovable_clone.service.DeploymentService;
...
/* Client calls Deploy API (projectId)
        ↓
Generate project subdomain
        ↓
Check if project already has a RUNNING pod
        ↓
IF YES:
    Return existing project URL
        ↓
IF NO:
    Find an IDLE pod from runner pool
        ↓
    If no idle pod → Throw "Scale up runner-pool"
        ↓
    Claim the pod:
        - Change label status: idle → busy
        - Add label project-id
        ↓
    Start Deployment Process (inside pod)
        ↓
        [SYNCER CONTAINER]
            1. Initial file sync from MinIO → /app
            2. Start background live sync (--watch)
        ↓
        [RUNNER CONTAINER]
            3. npm install
